...
const CONFIG = window.CONFIG || {
  estadosPermitidos: ['NUEVA', 'EN PROGRESO'], // Fallback mínimo
...
function setupEventListeners() {
  // Archivos - usando helper on() para manejo seguro
  ...
  // Filtros CATEC mutuamente excluyentes
  ...
  // Otros filtros con manejo seguro
  on('showAllStates', 'change', applyFilters);
  
  on('filterFTTH', 'change', e => {
    if (e.target.checked) {
      const el = document.getElementById('filterExcludeFTTH');
  ...
  on('filterExcludeFTTH', 'change', e => {
    if (e.target.checked) {
      const el = document.getElementById('filterFTTH');
      if (el) el.checked = false;
    }
    applyFilters();
  });
...
async function loadFile(e, tipo) {
  ...
  try {
    ...
    if (statusEl) statusEl.textContent = `✗ Error`;
    toast(`Error inesperado: ${error.message}`);
  }
}
...
// ... (código existente de stats) ...
  ...
  // === NUEVO CÓDIGO PARA FMS ===
  // Aplanamos todas las alarmas de las zonas filtradas para mostrarlas en la tabla general
  const allAlarmas = analyzed.flatMap(z => {
      // Inyectamos el nombre de la zona en cada objeto de alarma para mostrarlo en la tabla
      return (z.alarmas || []).map(a => ({ ...a, zonaTecnica: z.zona }));
  });
  
  // Filtramos según el filtro global de alarmas si es necesario
  let alarmasFiltradas = allAlarmas;
  if (Filters.alarma === 'con-alarma') {
      alarmasFiltradas = allAlarmas.filter(a => a.isActive);
  } else if (Filters.alarma === 'sin-alarma') {
      alarmasFiltradas = []; // Si filtra 'sin alarma', no mostramos alarmas FMS
  }
